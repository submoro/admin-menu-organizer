...
final class Detector {
	...
	public static function create(): Detector {
		 ...
		 */
		$known_slugs = apply_filters( 'mocam_known_slugs', require MOCAM_PLUGIN_DIR . 'includes/data/known-slugs.php' );
		 ...
		 */
		$heuristics = apply_filters( 'mocam_keyword_map', require MOCAM_PLUGIN_DIR . 'includes/data/keyword-map.php' );
		...
		$filter = static function ( string $group, array $item ): string {
			 ...
			 */
			return (string) apply_filters( 'mocam_resolve_group', $group, $item );
		};
	...
	public function explain( array $item ): array {
		...
		if ( null !== $this->filter ) {
			...
			if ( $filtered !== $result['group'] && $this->is_valid( $filtered ) ) {
				...
				$result['layer']    = 'filter';
				$result['evidence'] = 'mocam_resolve_group';
			}
		}
