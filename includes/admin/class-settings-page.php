 ...
 * manage_options, while a personal layout needs only mocam_personalise_menu, so a
...
final class Settings_Page {
	 ...
	 * @var string
	 */
	const NONCE_SITE = 'mocam_save_site';
	 ...
	 * @var string
	 */
	const NONCE_PERSONAL = 'mocam_save_personal';
	...
	public static function register(): void {
		...
		add_filter( 'plugin_action_links_' . MOCAM_PLUGIN_BASENAME, array( __CLASS__, 'action_links' ) );
	}
	...
	public static function add_page(): void {
		...
		add_options_page(
			__( 'Menu Organizer', 'menu-organizer-collapsible-admin-menu' ),
			__( 'Menu Organizer', 'menu-organizer-collapsible-admin-menu' ),
			$capability,
			self::SLUG,
	...
	public static function action_links( $links ): array {
		...
		$settings = sprintf(
			...
			esc_html__( 'Settings', 'menu-organizer-collapsible-admin-menu' )
		);
	...
	public static function enqueue( $hook_suffix ): void {
		...
		wp_enqueue_style(
			'mocam-settings',
			MOCAM_PLUGIN_URL . 'assets/css/settings.css',
			array( 'dashicons' ),
			MOCAM_VERSION
		);
		...
		wp_enqueue_script(
			'mocam-settings',
			MOCAM_PLUGIN_URL . 'assets/js/settings.js',
	...
	private static function tabs(): array {
		...
		if ( Capabilities::current_user_can_manage() ) {
			$tabs['layout']   = __( 'Layout', 'menu-organizer-collapsible-admin-menu' );
			$tabs['groups']   = __( 'Groups', 'menu-organizer-collapsible-admin-menu' );
			$tabs['roles']    = __( 'Roles', 'menu-organizer-collapsible-admin-menu' );
			$tabs['advanced'] = __( 'Advanced', 'menu-organizer-collapsible-admin-menu' );
		}

		if ( Capabilities::current_user_can_personalise() ) {
			$tabs['personal'] = __( 'Personalise my menu', 'menu-organizer-collapsible-admin-menu' );
		}
	...
	public static function render(): void {
		if ( ! Capabilities::current_user_can_manage() && ! Capabilities::current_user_can_personalise() ) {
			wp_die( esc_html__( 'You are not allowed to access this page.', 'menu-organizer-collapsible-admin-menu' ), 403 );
		}

		$tab    = self::current_tab();
		$reader = Menu_Reader::from_globals();

		require MOCAM_PLUGIN_DIR . 'includes/admin/views/page.php';
	}
	...
	public static function handle_post(): void {
		...
		$action = isset( $_POST['mocam_action'] ) ? sanitize_key( wp_unslash( $_POST['mocam_action'] ) ) : '';
	...
	private static function require_site_nonce(): void {
		...
		if ( ! Capabilities::current_user_can_manage() ) {
			wp_die( esc_html__( 'You are not allowed to change these settings.', 'menu-organizer-collapsible-admin-menu' ), 403 );
		}
	}
	...
	private static function require_personal_nonce(): void {
		...
		if ( ! Capabilities::current_user_can_personalise() ) {
			wp_die( esc_html__( 'You are not allowed to personalise your menu.', 'menu-organizer-collapsible-admin-menu' ), 403 );
		}
	}
	...
	private static function posted_layout(): array {
		...
		foreach ( self::posted_map( 'mocam_group_items' ) as $group_id => $slug_list ) {
			$groups[] = array(
				'id'    => $group_id,
	...
	private static function posted_groups( Menu_Reader $reader ): array {
		...
		$order  = self::posted_text( 'mocam_group_order' );
		$labels = self::posted_map( 'mocam_group_label' );
		$icons  = self::posted_map( 'mocam_group_icon' );
		$open   = self::posted_map( 'mocam_group_open' );
	...
	private static function save_roles( Menu_Reader $reader ): void {
		foreach ( self::posted_map( 'mocam_role_source' ) as $role => $source ) {
	...
	private static function save_settings(): void {
		...
		$settings['personalisation'] = isset( $_POST['mocam_personalisation'] );
	...
	private static function import_layout( Menu_Reader $reader ): bool {
		 ...
		 */
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Verified by the caller.
		$json = isset( $_POST['mocam_import'] )
			// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Verified by the caller.
			? trim( sanitize_textarea_field( wp_unslash( $_POST['mocam_import'] ) ) )
			: '';
	...
	public static function notice_for( string $status ): ?array {
		$notices = array(
			'saved'         => array(
				'message' => __( 'Your changes have been saved.', 'menu-organizer-collapsible-admin-menu' ),
				'type'    => 'success',
			),
			'reset'         => array(
				'message' => __( 'The layout has been reset to the automatically detected default.', 'menu-organizer-collapsible-admin-menu' ),
				'type'    => 'success',
			),
			'imported'      => array(
				'message' => __( 'The layout was imported.', 'menu-organizer-collapsible-admin-menu' ),
				'type'    => 'success',
			),
			'import-failed' => array(
				'message' => __( 'That layout could not be imported. Check that it is the JSON produced by the export button.', 'menu-organizer-collapsible-admin-menu' ),
				'type'    => 'error',
			),
		);
