# Claude Code Build Prompt - WordPress Archinest Sidebar Categories Plugin
...
- Proposed slug: `archinest-sidebar-categories`
- Proposed display name: `Archinest Sidebar Categories`
...
Location: `Settings > Sidebar Categories` (`options-general.php?page=amorg`)
...
- Every user-facing string wrapped in `__()`, `_e()`, `esc_html__()`, `esc_attr__()`, `_n()`, or `_x()` with the text domain `archinest-sidebar-categories`
...
- Generate `languages/archinest-sidebar-categories.pot` with WP-CLI: `wp i18n make-pot . languages/archinest-sidebar-categories.pot`
...
```php
/**
 * Plugin Name:       Archinest Sidebar Categories
 ...
 * Text Domain:       archinest-sidebar-categories
...
├── archinest-sidebar-categories.php   # bootstrap only, no logic
