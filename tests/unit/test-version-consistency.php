 ...
 * tag points at, regardless of what the header claims. AMORG_VERSION is a third
...
final class Test_Version_Consistency extends TestCase {
	...
	/**
	 * The plugin header, the AMORG_VERSION constant and the readme's stable tag
	...
	public function test_all_three_version_declarations_agree(): void {
		...
		$constant = $this->capture( "/define\(\s*'AMORG_VERSION',\s*'([^']+)'/", $plugin_file, 'the AMORG_VERSION constant' );
		$stable   = $this->capture( '/^Stable tag:\s*(.+)$/m', $readme, 'the readme stable tag' );

		$this->assertSame( $header, $constant, 'Plugin header Version and AMORG_VERSION disagree.' );
	...
	/**
	 * The AMORG_MIN_PHP and AMORG_MIN_WP constants match the declared headers,
	...
	public function test_runtime_guard_constants_match_the_headers(): void {
		...
		$header_wp   = $this->capture( '/^\s*\*\s*Requires at least:\s*(.+)$/m', $plugin_file, 'the Requires at least header' );
		$constant_wp = $this->capture( "/define\(\s*'AMORG_MIN_WP',\s*'([^']+)'/", $plugin_file, 'the AMORG_MIN_WP constant' );
