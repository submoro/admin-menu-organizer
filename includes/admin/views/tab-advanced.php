...
<h2><?php echo esc_html__( 'Reset', 'admin-menu-organizer' ); ?></h2>
...
<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=' . Settings_Page::SLUG . '&tab=advanced' ) ); ?>">
	...
	<p class="description">
		<?php echo esc_html__( 'Discards the site-wide arrangement and returns to the automatically detected grouping. Personal layouts and role presets are left alone.', 'admin-menu-organizer' ); ?>
	</p>

	<?php submit_button( __( 'Reset to detected default', 'admin-menu-organizer' ), 'secondary' ); ?>
</form>

<hr>

<h2><?php echo esc_html__( 'Export', 'admin-menu-organizer' ); ?></h2>

<p class="description">
	<?php echo esc_html__( 'Copy this to move the arrangement to another site.', 'admin-menu-organizer' ); ?>
</p>

<label for="amorg-export" class="screen-reader-text">
	<?php echo esc_html__( 'Layout as JSON', 'admin-menu-organizer' ); ?>
</label>
<textarea id="amorg-export" class="large-text code" rows="10" readonly><?php echo esc_textarea( (string) $amorg_export ); ?></textarea>

<hr>

<h2><?php echo esc_html__( 'Import', 'admin-menu-organizer' ); ?></h2>
...
<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=' . Settings_Page::SLUG . '&tab=advanced' ) ); ?>">
	...
	<p class="description">
		<?php echo esc_html__( 'Paste an exported layout. Anything it refers to that does not exist on this site is ignored rather than imported, so a layout from a site with different plugins is safe to use.', 'admin-menu-organizer' ); ?>
	</p>

	<label for="amorg-import" class="screen-reader-text">
		<?php echo esc_html__( 'Layout JSON to import', 'admin-menu-organizer' ); ?>
	</label>
	<textarea id="amorg-import" name="amorg_import" class="large-text code" rows="8"></textarea>

	<?php submit_button( __( 'Import layout', 'admin-menu-organizer' ), 'secondary' ); ?>
</form>

<hr>

<h2><?php echo esc_html__( 'If something goes wrong', 'admin-menu-organizer' ); ?></h2>

<p>
	<?php echo esc_html__( 'Two escape hatches always work, and both leave your saved arrangement intact:', 'admin-menu-organizer' ); ?>
</p>
...
<ol>
	<li>
		...
		printf(
			...
			esc_html__( 'Add %s to any admin URL to turn the plugin off for that one page load.', 'admin-menu-organizer' ),
			'<code>?amorg=off</code>'
		);
		?>
	</li>
	<li>
		...
		printf(
			...
			esc_html__( 'Add %1$s to %2$s to turn it off entirely.', 'admin-menu-organizer' ),
...
<h2><?php echo esc_html__( 'Why items were grouped this way', 'admin-menu-organizer' ); ?></h2>

<p class="description">
	<?php echo esc_html__( 'What the automatic rules decided for every top-level item currently in your sidebar, and which rule decided it. Anything you have moved by hand overrides this.', 'admin-menu-organizer' ); ?>
</p>

<table class="widefat striped">
	<caption class="screen-reader-text">
		<?php echo esc_html__( 'Automatic grouping decisions', 'admin-menu-organizer' ); ?>
	</caption>
	<thead>
		<tr>
			<th scope="col"><?php echo esc_html__( 'Menu item', 'admin-menu-organizer' ); ?></th>
			<th scope="col"><?php echo esc_html__( 'Slug', 'admin-menu-organizer' ); ?></th>
			<th scope="col"><?php echo esc_html__( 'Detected group', 'admin-menu-organizer' ); ?></th>
			<th scope="col"><?php echo esc_html__( 'Decided by', 'admin-menu-organizer' ); ?></th>
			<th scope="col"><?php echo esc_html__( 'Matched on', 'admin-menu-organizer' ); ?></th>
		</tr>
	</thead>
	<tbody>
...
<p class="description">
	...
	printf(
		...
		esc_html__( '%1$d of %2$d items were grouped automatically.', 'admin-menu-organizer' ),
		(int) count(
			array_filter(
				$amorg_items,
