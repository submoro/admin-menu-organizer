...
Writes `build/menu-organizer-collapsible-admin-menu.<version>.zip`, taking its
...
`https://plugins.svn.wordpress.org/menu-organizer-collapsible-admin-menu/`.

```bash
svn checkout https://plugins.svn.wordpress.org/menu-organizer-collapsible-admin-menu/ mocam-svn
cd mocam-svn
```
...
rm -rf trunk/*
unzip -j -o /path/to/menu-organizer-collapsible-admin-menu.1.0.0.zip -d /tmp/mocam
cp -R /tmp/mocam/menu-organizer-collapsible-admin-menu/* trunk/
...
header and `MOCAM_VERSION` all agree.

## Subsequent releases

1. Bump the version in **both** the plugin header and `MOCAM_VERSION`, and the
