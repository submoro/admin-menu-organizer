 ...
 *
 * Archinest Sidebar Categories is free software: you can redistribute
...
function amorg_render_requirement_notice() {
	...
	if ( 'php' === $unmet ) {
		$message = sprintf(
			...
			__( 'Archinest Sidebar Categories requires PHP %1$s or newer. This site is running PHP %2$s, so the plugin has not been loaded.', 'archinest-sidebar-categories' ),
			AMORG_MIN_PHP,
			PHP_VERSION
		);
	} else {
		$message = sprintf(
			...
			__( 'Archinest Sidebar Categories requires WordPress %1$s or newer. This site is running WordPress %2$s, so the plugin has not been loaded.', 'archinest-sidebar-categories' ),
			AMORG_MIN_WP,
