...
<?php if ( $amorg_personal ) : ?>
	<p class="amorg-intro">
		<?php echo esc_html__( 'Arrange your own sidebar. This affects only your account; everyone else keeps the site default.', 'archinest-sidebar-categories' ); ?>
	</p>
<?php else : ?>
	<p class="amorg-intro">
		<?php echo esc_html__( 'Drag items between groups to arrange the sidebar for everyone. Every item stays reachable: anything you do not place appears in the Other group at the bottom.', 'archinest-sidebar-categories' ); ?>
	</p>
<?php endif; ?>
...
<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=' . Settings_Page::SLUG . '&tab=' . $tab ) ); ?>" class="amorg-editor-form">
			...
			<section class="amorg-group" data-amorg-group="<?php echo esc_attr( $amorg_id ); ?>">
				<h2 class="amorg-group-title">
					...
					<?php if ( Categories::UNGROUPED === $amorg_id ) : ?>
						<span class="amorg-badge-permanent"><?php echo esc_html__( 'always visible', 'archinest-sidebar-categories' ); ?></span>
					<?php endif; ?>
				</h2>
				...
				<ul
					...
					echo esc_attr(
						sprintf(
							...
							__( 'Items in %s', 'archinest-sidebar-categories' ),
							$amorg_label
						)
					);
					?>
								...
								<span class="screen-reader-text">
									...
									echo esc_html(
										sprintf(
											...
											__( 'Move %s to group', 'archinest-sidebar-categories' ),
											$amorg_item['label']
										)
									);
									?>
	...
	<p class="submit">
		<?php submit_button( __( 'Save layout', 'archinest-sidebar-categories' ), 'primary', 'submit', false ); ?>
		...
		<?php if ( $amorg_personal && Layout_Repository::has_user_layout( get_current_user_id() ) ) : ?>
			...
			><?php echo esc_html__( 'Reset to site default', 'archinest-sidebar-categories' ); ?></button>
		<?php endif; ?>
	</p>
</form>
