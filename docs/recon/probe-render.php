...
probe( 'CLAIM 1 - separator-class item WITH a title and icon', array(
	array( 'MY GROUP HEADER', 'read', 'mocam-group-content', '', 'mocam-group-header wp-menu-separator', 'mocam-hook', 'dashicons-admin-post' ),
) );
...
probe( 'CLAIM 3 - empty slug, custom class, no submenu', array(
	array( 'MY GROUP HEADER', 'read', '', '', 'mocam-group-header mocam-group-content', 'mocam_group_content', '' ),
) );
...
probe( 'CLAIM 5 - item whose capability the user lacks', array(
	array( 'Secret', 'do_not_allow', 'secret.php', '', 'mocam-item', '', '' ),
) );
