...
1. Download the latest `archinest-sidebar-categories.<version>.zip` from
   [Releases](../../releases).
...
4. Your sidebar is grouped immediately, using the automatic rules. Nothing to
   configure.
5. To rearrange anything, go to **Settings → Sidebar Categories**.
...
cd archinest-sidebar-categories
composer install
composer run build
```

That runs the full gate — linting, tests, security audit, readme validation —
then writes `build/archinest-sidebar-categories.<version>.zip`. Upload that as above.

### Installing directly into a site

If you have filesystem access, clone straight into your plugins directory. The
repository folder name must be `archinest-sidebar-categories`, because WordPress derives
...
```bash
wp plugin install /path/to/archinest-sidebar-categories.1.0.0.zip --activate
```
