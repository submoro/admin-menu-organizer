/**
 * Sidebar Categories - settings screen editor.
 *
...
( function ( $ ) {
				...
				if ( ! $empty.length ) {
					$group.find( '.amorg-droplist' ).after(
						$( '<p/>', {
							...
							text: __( 'Empty. This group will not appear in the sidebar.', 'archinest-sidebar-categories' )
						} )
					);
				}
			} else {
	...
	function moveChip( slug, groupId ) {
		...
		announce(
			__( 'Moved to group', 'archinest-sidebar-categories' ) + ': ' + $target.closest( '.amorg-group' ).find( '.amorg-group-title' ).text().trim()
		);
	}
